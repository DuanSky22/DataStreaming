...
object SparkWindowJoin {
  ...
  def main(args:Array[String]): Unit ={
    ...
    val aMap = getStream(topics(0),ssc).window(Seconds(JoinConfig.sparkStreamingDuration * JoinConfig.aLengthTimes))
    val bMap = getStream(topics(1),ssc).window(Seconds(JoinConfig.sparkStreamingDuration * JoinConfig.bLengthTimes))
