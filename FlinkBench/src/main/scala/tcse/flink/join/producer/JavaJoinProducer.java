...
public class JavaJoinProducer {
    ...
    public JavaJoinProducer(){
        ...
        //此处配置的是kafka的端口
        props.put("bootstrap.servers", JoinConfig.brokers());
    ...
    public void produce() {
        ...
        while(aPos != aKeys.size() && bPos != bKeys.size()){
            if(aPos != aKeys.size()){
                ...
                producer.send(new ProducerRecord<String, String>(topics[0], key ,data));
            ...
            if(bPos != bKeys.size()){
                ...
                producer.send(new ProducerRecord<String, String>(topics[1], key ,data));
